...
fn _lex_symbol(ctx: &mut Cursor) {
    let mut symbols: &Branch = &symbol::MAP;
    let mut options = vec![];

    while let Some(c) = ctx.peek() {
        // check the leaves to see if any of the cadences or special conditions match.
        'sym: for symbol in &symbols.leaves {
            if let Some(checks) = &symbol.checks() {
                for check in checks {
                    if !check.vs(ctx) {
                        continue;
                    }
            ...
            for cadence in symbol.cads() {
                if ctx.peeked_has(*cadence) {
                    options.push(symbol);
                    continue 'sym;
                }
            }
        ...
        if options.is_empty() {
            if let Some(branch) = symbols.branches.get(&c) {
                symbols = branch;
            } else {
                todo!("Unknown symbol: {}", ctx.peeked());
            }
        }
        // found!
        else if options.len() == 1 {
            let symbol = options.pop().unwrap();
            ctx.start_term(Term::Type::Symbol(
                symbol.name().to_string(),
                *symbol.kind(),
            ));
            ctx.read_(symbol.chars().len());
            ctx.end_term();
            return;
        }
        // ambiguous
        else {
            todo!("ambiguous symbols: {:?}", options);
        }
    }
}
